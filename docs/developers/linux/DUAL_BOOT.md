...
3. The Debian vmlinuz kernel is loaded at `6.12.0-kurono` (matching the Trixie kernel version).
4. The APT source list in the guest points at `deb.debian.org/debian trixie main`.
5. The guest Linux boots inside the Kurono hypervisor and appears in the Conduit app.

## 3. Debian version

The module is configured for Debian 13 (Trixie) 13.4:

- `debian_version = 13.4`
- apt sources: `deb http://deb.debian.org/debian trixie main contrib non-free`
- boot kernel: `vmlinuz-6.12.0-kurono`
- initrd: `initrd.img-6.12.0-kurono`
...
- `src/apps/conduit.cpp`  -  app surface where the Linux guest appears
