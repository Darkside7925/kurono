...
> **On the browser question:** Kurono does *not* ship a cut-down toy browser, and the GUI "Browser" tile is a deliberate placeholder. The real answer is **Firefox on the Linux runtime**, and it has crossed the line from "the engine runs" to **Firefox painting its real browser chrome on the Kurono desktop**: a full Firefox 140.11.0esr (musl + Wayland, 174 MB `libxul.so`, software WebRender) boots in-process across the secondary cores, loads a real profile, runs BrowserGlue and the services JS, and composites its actual UI - tab strip, URL bar, back/forward/reload, the account/extensions/hamburger menus - through the in-kernel compositor's **`wl_subsurface` compositing** path (the GTK content surface is composited onto the CSD toplevel at its `set_position` offset). The old blockers died one by one: not a bind-id desync, not e10s (Kurono runs Firefox single-process), not an address-space limit. What remains before a **painted web page** is the last stretch of the network bring-up (navigation reaches necko; the socket thread's event-wakeup is the active fix) plus render-timing flakiness on some boots. `curl <url>` is still the reliable HTTP path today.
...
### Latest (July 2026): Firefox paints its browser chrome + an SMP correctness hardening pass

Getting Firefox from "full Wayland handshake" to **its real UI composited on screen** turned into the most productive kernel-debugging stretch this project has had. Every one of these is a genuine, reusable kernel fix that Firefox merely happened to be the first program brutal enough to expose:
...
> **Honest status of the Firefox work:** the *chrome* renders reliably enough to screenshot, and the eight items above are durable kernel fixes that stand on their own regardless of Firefox. A rendered **web page** is not yet guaranteed on every boot: navigation now reaches necko, but the socket thread's event-wakeup delivery is still being closed out and some boots hit render-timing flakiness. The Firefox-config prefs and the debug-probe scaffolding used to find these bugs are still being trimmed out of the tree.
