...
> **On the browser question:** Kurono does *not* ship a cut-down toy browser, and the GUI "Browser" tile is a deliberate placeholder. The real answer is **Firefox on the Linux runtime**: a full Firefox 140.11.0esr is cross-compiled against musl + Wayland (174 MB `libxul.so`), and the OS provides the syscalls, IPC and Wayland surface it targets. The **Gecko engine loads and runs on-device**, and the runtime has since been driven all the way to a **full Wayland client handshake**: Firefox connects to the in-kernel compositor, its WaylandProxy forwards `get_registry`, it binds all 9 advertised globals, and it runs `wl_compositor.create_surface`, a `wl_shm` buffer pool, and `wl_seat`/`wl_output` setup with zero faults and no protocol errors. The one remaining blocker before a **rendered Firefox window** is a compositor bind-id desync (firefox's `wl_registry.bind` carries no object-id on the wire, so the compositor's id guess diverges from firefox's), *not* an e10s/IPC limit and *not* an address-space limit (the old `<4 GB` pointer-ABI cap is gone; user mappings now span the full canonical 64-bit user range). `curl <url>` is the working HTTP path today.
...
### Latest: KMemX - native memory compression engine
