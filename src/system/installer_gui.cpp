...
static void scr_keyboard(){
    ...
    if (draw_button(g_hit.next_x, g_hit.next_y, g_hit.next_w, g_hit.next_h, "Next", 0xFF5C8AFF, true)){
        ...
        InstallerGUI::current_screen = InstallerGUI::SCR_DISK;
    }
}
...
static void scr_summary(){
    ...
    int py = 180;
    Graphics::FillRoundedRect(panel_x, py, panel_w, 320, 14, 0xCC141428);
    ...
    row(py + 32, "Language:",  LANGUAGES[g_lang_idx]);
    row(py + 56, "Keyboard:",  KBDS[g_kbd_idx]);
    row(py + 80, "Disk:",      d ? (d->model[0]?d->model:d->name) : "(none)");
    row(py + 104, "Disk size:", szs);
    row(py + 128, "Partition:", g_part_mode == 0 ? "Erase entire disk (GPT)" : "Use existing partitions");
    row(py + 152, "Filesystem:", g_fs_idx == 0 ? "ext4" : "FAT32");
    row(py + 176, "Username:",  g_user_name);
    row(py + 200, "Display:",   g_user_disp);

    if (g_part_mode == 0){
        FontTTF::DrawString(panel_x + 24, py + 248, 13.0f,
...
static void install_step(){
    ...
    switch (g_install_step){
        ...
        case 2: {
            ...
            if (p < 0){
                scpy(g_install_log,
                    "No ext4 partition found.  Pre-create an ESP and ext4 partition or run 'installer' to plan.",
                    256);
        ...
        case 5: {
            scpy(InstallerGUI::progress_status, "Writing installation marker...", 128);
            InstallerGUI::progress_pct = 95;
            // Write the marker with the timestamp and language so the next
            // boot knows we're already provisioned.
            RTC::Date d; RTC::Time t; RTC::ReadDateTime(d, t);
