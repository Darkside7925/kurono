...
static float g_fade = 0.0f;             // 0..1 (eased)
static float g_fade_raw = 0.0f;         // linear 0..1 progress used to compute eased fade
static int   g_shake_t = 0;             // ms remaining
static int   g_shake_total = 0;         // total ms of current shake
...
// step slide animation now driven by ms instead of raw "subtract dt" hack.
// g_step_anim_dir: -1 = sliding in from the left, +1 = sliding in from the right,
//                   0 = settled.  g_step_anim_t counts down from WIZ_ANIM_MS.
static int   g_step_anim_dir = 0;
static int   g_step_anim_t = 0;
static const int WIZ_ANIM_MS = 240;

// per-dot pop animation timestamps (ms remaining; 0 = settled)
static int   g_login_dot_pop[64] = {0};
static int   g_reg_pw_dot_pop[64] = {0};
static int   g_reg_cf_dot_pop[64] = {0};
static int   g_reg_pin_dot_pop[16] = {0};
static const int DOT_POP_MS = 220;

// clock pulse: triggered when the second flips. Range 0..1, decays to 0.
static int   g_last_sec = -1;
static int   g_last_min = -1;
static int   g_clock_pulse_t = 0;    // ms remaining on the clock-second pulse
static const int CLOCK_PULSE_MS = 700;

// cached clock string ("HH:MM"). Re-formatted only when the minute changes.
static char  g_clock_hm[6]   = {0};
static char  g_clock_ss[3]   = {0};
static char  g_date_str[64]  = {0};

// frame-time accumulator
static int g_frame_accum = 0;
...
static void int2(int n, char* out){
    if (n < 0) n = 0; if (n > 99) n = 99;
    out[0]=(char)('0'+(n/10)%10); out[1]=(char)('0'+n%10); out[2]=0;
}
static void itoa_pos(int n, char* out){
    if (n<=0){ out[0]='0'; out[1]=0; return; }
    char tmp[12]; int i=0; while (n>0 && i<11){ tmp[i++]=(char)('0'+n%10); n/=10; }
    int o=0; while (i>0) out[o++]=tmp[--i]; out[o]=0;
}

// ---------------------------------------------------- easing
// cubic ease-out: smooth deceleration; quint ease-out for snappier feel.
...
// Password dots with per-character spring scale. dot_pop[i] is ms remaining of pop.
static void draw_password_dots(int x, int y_baseline, float fs, int n, const int* dot_pop, uint32_t color){
    if (n <= 0) return;
    float adv = fs * 0.55f;
    int row_y = y_baseline - (int)(fs * 0.45f);
    int radius_base = (int)(fs * 0.22f);
    if (radius_base < 3) radius_base = 3;
    for (int i = 0; i < n; i++){
        int cx = x + (int)(adv * (float)i + adv * 0.5f);
        int cy = row_y;
        float scale = 1.0f;
        int pop_left = dot_pop[i];
        if (pop_left > 0){
            float t = 1.0f - ((float)pop_left / (float)DOT_POP_MS);
            scale = 0.20f + 0.80f * ease_out_back(t);
            if (scale < 0.20f) scale = 0.20f;
            if (scale > 1.18f) scale = 1.18f;
        }
        int r = (int)((float)radius_base * scale);
        if (r < 1) r = 1;
        Graphics::FillCircle(cx, cy, r, color);
    }
}

// frosted-glass pill input
// dot_pop is optional (null if not a password field with per-dot animation).
static void draw_pill(int x, int y, int w, int h, const char* text, int text_len, bool focused,
                      bool is_password, const char* placeholder, const int* dot_pop){
    ...
    int pad = h/2;
    int baseline_y = y + (int)(fs*1.1f);
    if (text_len > 0){
        if (is_password){
            int n = text_len < 60 ? text_len : 60;
            if (dot_pop){
                draw_password_dots(x + pad, baseline_y, fs, n, dot_pop, 0xFFEEEEFF);
            } else {
                char dots[64];
                for (int i=0;i<n;i++) dots[i] = '*';
                dots[n] = 0;
                FontTTF::DrawString(x + pad, baseline_y, fs, dots, 0xFFEEEEFF);
            }
        } else {
            FontTTF::DrawString(x + pad, baseline_y, fs, text, 0xFFEEEEFF);
        }
    } else if (placeholder){
        FontTTF::DrawString(x + pad, baseline_y, fs, placeholder, 0xFF8090A0);
    }
    if (focused && g_blink_on){
        int tw;
        if (is_password) tw = text_len * (int)(fs*0.55f);
        else if (text_len > 0) tw = FontTTF::Measure(fs, text);
        else tw = 0;
        int cx = x + pad + tw + 2;
...
// ---------------------------------------------------- clock cache
// rebuild cached clock strings if minute/second changed; returns true if seconds rolled
static bool refresh_clock_cache(){
    RTC::Date dr; RTC::Time tr; RTC::ReadDateTime(dr, tr);
    bool sec_changed = (tr.s != g_last_sec);
    bool min_changed = (tr.m != g_last_min);
    if (min_changed){
        int2(tr.h, g_clock_hm); g_clock_hm[2]=':'; int2(tr.m, g_clock_hm+3); g_clock_hm[5]=0;
        // date string
        int p = 0;
        const char* dn = (dr.dow >= 1 && dr.dow <= 7) ? DOW[dr.dow-1] : "Day";
        const char* mn = (dr.mon >= 1 && dr.mon <= 12) ? MON[dr.mon-1] : "Mon";
        while (*dn && p < 60) g_date_str[p++] = *dn++;
        if (p < 62){ g_date_str[p++] = ','; g_date_str[p++] = ' '; }
        while (*mn && p < 60) g_date_str[p++] = *mn++;
        if (p < 63) g_date_str[p++] = ' ';
        char dom[4]; itoa_pos(dr.dom, dom);
        for (int i=0; dom[i] && p < 60; i++) g_date_str[p++] = dom[i];
        if (p < 63) g_date_str[p++] = ' ';
        char yr[8]; itoa_pos((int)dr.year, yr);
        for (int i=0; yr[i] && p < 63; i++) g_date_str[p++] = yr[i];
        g_date_str[p] = 0;
        g_last_min = tr.m;
    }
    if (sec_changed){
        int2(tr.s, g_clock_ss);
        g_last_sec = tr.s;
        if (g_last_min >= 0) g_clock_pulse_t = CLOCK_PULSE_MS;
    }
    return sec_changed;
}
...
static void draw_idle(){
    ...
    // subtle pulse: brighten clock briefly when the second flips
    float pulse = (g_clock_pulse_t > 0) ? ((float)g_clock_pulse_t / (float)CLOCK_PULSE_MS) : 0.0f;
    float ep = ease_out_cubic(pulse);
    uint8_t pulse_alpha = (uint8_t)(ep * 40.0f);
    uint32_t hm_color = 0xFFFFFFFF;

    float fs_big = (float)g_h * 0.18f;
    if (fs_big > 240.0f) fs_big = 240.0f;
    int tw = FontTTF::Measure(fs_big, g_clock_hm);
    int ty = g_h/2 - (int)(fs_big*0.6f);
    FontTTF::DrawString(cx - tw/2, ty, fs_big, g_clock_hm, hm_color);
    if (pulse_alpha > 0){
        // very thin highlight overlay band behind the clock numerals
        int bh = (int)(fs_big * 0.9f);
        Graphics::FillRectAlpha(cx - tw/2 - 24, ty - (int)(fs_big*0.55f), tw + 48, bh, pulse_alpha, 0x00FFFFFF);
    }

    float fs_sec = fs_big * 0.30f;
    int sw = FontTTF::Measure(fs_sec, g_clock_ss);
    FontTTF::DrawString(cx - sw/2, ty + (int)(fs_big*0.6f) + 4, fs_sec, g_clock_ss, 0xC0FFFFFF);

    float fs_date = fs_big * 0.20f;
    int dw = FontTTF::Measure(fs_date, g_date_str);
    FontTTF::DrawString(cx - dw/2, ty + (int)(fs_big*0.6f) + (int)(fs_sec*1.4f) + 18, fs_date, g_date_str, 0xCCFFFFFF);
...
static void draw_login(int shake_offset_x){
    ...
    if (user_count > 0 && g_active_user_idx >= user_count) g_active_user_idx = 0;
    User* u = (user_count > 0) ? &UserManager::users[g_active_user_idx] : nullptr;

    // clock at the top - cached
    float fs_clk = 80.0f;
    int twc = FontTTF::Measure(fs_clk, g_clock_hm);
    FontTTF::DrawString(cx - twc/2, 90, fs_clk, g_clock_hm, 0xFFFFFFFF);
    ...
    uint32_t panel_accent = u ? (u->accent_color ? u->accent_color : 0xFF5C8AFF) : 0xFF5C8AFF;
    draw_glass_panel(cx - 230, avatar_cy - 90, 460, 332, panel_accent);
    ...
    if (user_count == 0){
        ...
        draw_pill(g_login_hit.user_x, g_login_hit.user_y, field_w, field_h, g_login_user, g_login_user_len, !g_pw_focused, false, "Username", nullptr);
        fy += field_h + 12;
    } else {
        scpy(g_login_user, u->username, 32);
        g_login_user_len = slen(g_login_user);
        g_login_hit.user_w = 0;
    }

    g_login_hit.pw_x = cx - field_w/2; g_login_hit.pw_y = fy;
    g_login_hit.pw_w = field_w; g_login_hit.pw_h = field_h;
    draw_pill(g_login_hit.pw_x, g_login_hit.pw_y, field_w, field_h, g_login_pw, g_login_pw_len, g_pw_focused,
              !g_show_password, "Password", g_show_password ? nullptr : g_login_dot_pop);
    ...
    const char* ca = (user_count == 0) ? "" : "Create new account";
    if (ca[0]){
        float fs_ca = 13.0f;
...
static void wiz_reset(){
    ...
    for (int i=0;i<64;i++){ g_reg_pw_dot_pop[i] = 0; g_reg_cf_dot_pop[i] = 0; }
    for (int i=0;i<16;i++) g_reg_pin_dot_pop[i] = 0;
}
...
static void draw_wizard_step1(int slide_dx){
    ...
    draw_pill(g_wiz.display_x, fy, field_w, field_h, g_reg.display_name, g_reg.display_len,
              g_reg.active_field == 0, false, "Display name", nullptr);
    ...
    draw_pill(g_wiz.user_x, fy, field_w, field_h, g_reg.username, g_reg.user_len,
              g_reg.active_field == 1, false, "Username (lowercase, 3-31)", nullptr);
...
static void draw_wizard_step2(int slide_dx){
    ...
    draw_pill(g_wiz.pw_x, fy, field_w, field_h, g_reg.password, g_reg.pw_len,
              ...
              g_show_password ? nullptr : g_reg_pw_dot_pop);
    ...
    if (strength < 0) strength = 0; if (strength > 3) strength = 3;
    ...
    draw_pill(g_wiz.cf_x, fy, field_w, field_h, g_reg.confirm, g_reg.cf_len,
              ...
              g_show_password ? nullptr : g_reg_cf_dot_pop);
    if (g_reg.cf_len > 0){
    ...
    draw_pill(g_wiz.pin_x, g_wiz.pin_y, g_wiz.pin_w, g_wiz.pin_h, g_reg.pin, g_reg.pin_len,
              g_reg.active_field == 2, true, "PIN", g_reg_pin_dot_pop);
...
// ---------------------------------------------------- INPUT HANDLERS
// edit_buf returns: -1 on backspace consumed, 0 if ignored, +1 if char appended (with index in *out_idx).
static int edit_buf(char* buf, int* len, int max, char c, int* out_idx){
    if (!buf || !len || max < 1) return 0;
    if (c == '\b' || c == 127){
        if (*len > 0){ (*len)--; buf[*len]=0; return -1; }
        return 0;
    }
...
static void wizard_go_next(){
    ...
    if (LockScreen::current_state == LockScreen::WIZ_PROFILE){
        if (g_reg.display_len == 0){
            ...
            g_reg.display_len = slen(g_reg.display_name);
        }
        LockScreen::current_state = LockScreen::WIZ_SECURITY;
        g_reg.active_field = 0;
        g_fade_raw = 0.0f; g_fade = 0.0f;
        g_step_anim_dir = +1; g_step_anim_t = WIZ_ANIM_MS;
        return;
    }
    ...
    if (LockScreen::current_state == LockScreen::WIZ_SUMMARY){
        ...
        if (UserManager::RegisterUser(u, g_reg.password)){
            ...
            // wipe sensitive plaintext from RAM before transitioning out
            for (int i = 0; i < 64; i++){ g_reg.password[i] = 0; g_reg.confirm[i] = 0; }
            for (int i = 0; i < 16; i++) g_reg.pin[i] = 0;
            g_reg.pw_len = 0; g_reg.cf_len = 0; g_reg.pin_len = 0;
            LockScreen::current_state = LockScreen::FADE_OUT;
            g_fade_raw = 1.0f; g_fade = 1.0f;
        }
    }
}

// ---------------------------------------------------- char append helpers
// Trigger the per-dot pop animation at the appended index, with bounds checks.
static void trigger_pop(int* pop_arr, int max, int idx){
    if (idx < 0 || idx >= max) return;
    pop_arr[idx] = DOT_POP_MS;
}

// Decay all dot pop timers and the clock pulse by dt ms.
static void tick_animations(int dt){
    if (dt <= 0) return;
    for (int i = 0; i < 64; i++){
        if (g_login_dot_pop[i] > 0){ g_login_dot_pop[i] -= dt; if (g_login_dot_pop[i] < 0) g_login_dot_pop[i] = 0; }
        if (g_reg_pw_dot_pop[i]  > 0){ g_reg_pw_dot_pop[i]  -= dt; if (g_reg_pw_dot_pop[i]  < 0) g_reg_pw_dot_pop[i]  = 0; }
        if (g_reg_cf_dot_pop[i]  > 0){ g_reg_cf_dot_pop[i]  -= dt; if (g_reg_cf_dot_pop[i]  < 0) g_reg_cf_dot_pop[i]  = 0; }
    }
    for (int i = 0; i < 16; i++){
        if (g_reg_pin_dot_pop[i] > 0){ g_reg_pin_dot_pop[i] -= dt; if (g_reg_pin_dot_pop[i] < 0) g_reg_pin_dot_pop[i] = 0; }
    }
    if (g_clock_pulse_t > 0){ g_clock_pulse_t -= dt; if (g_clock_pulse_t < 0) g_clock_pulse_t = 0; }
}
...
void LockScreen::Show(){
    ...
    // hard-reset all transient state so re-entering Show() never carries
    // over a half-finished animation or stale buffers.
    g_fade = 0.0f;
    g_fade_raw = 0.0f;
    g_shake_t = 0;
    g_shake_total = 0;
    ...
    for (int i = 0; i < 64; i++){
        g_login_dot_pop[i] = 0;
        g_reg_pw_dot_pop[i] = 0;
        g_reg_cf_dot_pop[i] = 0;
    }
    for (int i = 0; i < 16; i++) g_reg_pin_dot_pop[i] = 0;
    ...
    // dirty tracking: when nothing visible changed we still draw at the
    // fixed cadence, but cheap inputs poll every iteration for snap response.
    while (g_running){
        InputManager::Poll();
        uint32_t dt = Timer::ElapsedSinceLast();
        // cap dt to avoid pathological jumps when the OS was paused (debugger, IRQ flood)
        if (dt > 200) dt = 200;
        ...
        // -- fades & state-driven anims (time-driven, not frame-counter-driven)
        if (current_state == LOGIN || (current_state >= WIZ_PROFILE && current_state <= WIZ_SUMMARY)){
            if (g_fade_raw < 1.0f){
                g_fade_raw += (float)dt / (float)WIZ_ANIM_MS;
                if (g_fade_raw > 1.0f) g_fade_raw = 1.0f;
                g_fade = ease_out_cubic(g_fade_raw);
            }
        }
        if (current_state == FADE_OUT){
            // slide+fade out using ease-out quint over 400ms
            g_fade_raw -= (float)dt / 400.0f;
            if (g_fade_raw < 0.0f) g_fade_raw = 0.0f;
            g_fade = ease_out_quint(g_fade_raw);
            if (g_fade_raw <= 0.0f){ g_running = false; }
        }
        if (current_state == SHAKE){
            g_shake_t -= (int)dt;
            if (g_shake_t <= 0){ g_shake_t = 0; current_state = LOGIN; }
        }
        if (g_step_anim_t > 0){
            g_step_anim_t -= (int)dt;
            if (g_step_anim_t <= 0){ g_step_anim_t = 0; g_step_anim_dir = 0; }
        }

        tick_animations((int)dt);
        bool sec_flipped = refresh_clock_cache();
        (void)sec_flipped;

        // ---------- IDLE
        if (current_state == IDLE){
            if (clicked || enter_pressed || Keyboard::HasChar()){
                // drain any char so it isn't double-applied as the first password char
                ...
                if (UserManager::GetUserCount() > 0){
                    ...
                    g_pw_focused = true;
                    g_login_pw[0] = 0; g_login_pw_len = 0;
                    for (int i = 0; i < 64; i++) g_login_dot_pop[i] = 0;
                } else {
                    wiz_reset();
                    current_state = WIZ_PROFILE;
                    g_fade_raw = 0.0f; g_fade = 0.0f;
        ...
        if (current_state == LOGIN){
            while (Keyboard::HasChar()){
                ...
                if (c == '\t'){
                    if (UserManager::GetUserCount() == 0) g_pw_focused = !g_pw_focused;
                    continue;
                }
                if (c == '\n' || c == '\r'){
                    bool ok = UserManager::Login(g_login_user, g_login_pw);
                    if (ok){
                        // scrub password from memory before transitioning
                        for (int i = 0; i < 64; i++) g_login_pw[i] = 0;
                        g_login_pw_len = 0;
                        current_state = FADE_OUT;
                        g_fade_raw = 1.0f; g_fade = 1.0f;
                    } else {
                        current_state = SHAKE;
                        g_shake_t = 320; g_shake_total = 320;
                        for (int i = 0; i < 64; i++){ g_login_pw[i] = 0; g_login_dot_pop[i] = 0; }
                        g_login_pw_len = 0;
                    }
                    continue;
                }
                if (g_pw_focused){
                    int idx = -1;
                    int r = edit_buf(g_login_pw, &g_login_pw_len, 64, c, &idx);
                    if (r > 0) trigger_pop(g_login_dot_pop, 64, idx);
                    else if (r < 0 && g_login_pw_len < 64) g_login_dot_pop[g_login_pw_len] = 0;
                } else if (UserManager::GetUserCount() == 0) {
                    edit_buf(g_login_user, &g_login_user_len, 32, c, nullptr);
                }
            }
            if (clicked){
                ...
                if (dx*dx + dy*dy < 100) g_show_password = !g_show_password;
                    ...
                    int n = UserManager::GetUserCount();
                    if (n > 0) g_active_user_idx = (g_active_user_idx + 1) % n;
                    for (int i = 0; i < 64; i++){ g_login_pw[i] = 0; g_login_dot_pop[i] = 0; }
                    g_login_pw_len = 0;
                }
        ...
        if (current_state >= WIZ_PROFILE && current_state <= WIZ_SUMMARY){
            ...
            while (Keyboard::HasChar()){
                ...
                if (c == '\n' || c == '\r'){
                    // Already handled by IsKeyPressed above; drain the buffered echo so it
                    // doesn't get appended as a literal newline character.
                    continue;
                }
                if (current_state == WIZ_PROFILE){
                    if (g_reg.active_field == 0) edit_buf(g_reg.display_name, &g_reg.display_len, 48, c, nullptr);
                    else                          edit_buf(g_reg.username,     &g_reg.user_len,    32, c, nullptr);
                } else if (current_state == WIZ_SECURITY){
                    if (g_reg.active_field == 0){
                        int idx = -1;
                        int r = edit_buf(g_reg.password, &g_reg.pw_len, 64, c, &idx);
                        if (r > 0) trigger_pop(g_reg_pw_dot_pop, 64, idx);
                        else if (r < 0 && g_reg.pw_len < 64) g_reg_pw_dot_pop[g_reg.pw_len] = 0;
                    } else if (g_reg.active_field == 1){
                        int idx = -1;
                        int r = edit_buf(g_reg.confirm, &g_reg.cf_len, 64, c, &idx);
                        if (r > 0) trigger_pop(g_reg_cf_dot_pop, 64, idx);
                        else if (r < 0 && g_reg.cf_len < 64) g_reg_cf_dot_pop[g_reg.cf_len] = 0;
                    } else if (g_reg.active_field == 2){
                        if ((c >= '0' && c <= '9') && g_reg.pin_len < 8){
                            int idx = -1;
                            int r = edit_buf(g_reg.pin, &g_reg.pin_len, 16, c, &idx);
                            if (r > 0) trigger_pop(g_reg_pin_dot_pop, 16, idx);
                        } else if (c == '\b' || c == 127){
                            edit_buf(g_reg.pin, &g_reg.pin_len, 16, c, nullptr);
                            if (g_reg.pin_len < 16) g_reg_pin_dot_pop[g_reg.pin_len] = 0;
                        }
                    }
                }
            }
            if (clicked){
                ...
                else if (g_wiz.back_w > 0 && point_in(mx,my,g_wiz.back_x,g_wiz.back_y,g_wiz.back_w,g_wiz.back_h)){
                    wizard_go_back();
                }
                else if (current_state == WIZ_PROFILE){
                ...
                else if (current_state == WIZ_PREFS){
                ...
                else if (current_state == WIZ_SUMMARY){
                    if (point_in(mx,my,g_wiz.edit_x[0],g_wiz.edit_y[0],g_wiz.edit_w,g_wiz.edit_h)){ current_state = WIZ_PROFILE; g_step_anim_dir = -1; g_step_anim_t = WIZ_ANIM_MS; g_fade_raw = 0.0f; g_fade = 0.0f; }
                    else if (point_in(mx,my,g_wiz.edit_x[1],g_wiz.edit_y[1],g_wiz.edit_w,g_wiz.edit_h)){ current_state = WIZ_SECURITY; g_step_anim_dir = -1; g_step_anim_t = WIZ_ANIM_MS; g_fade_raw = 0.0f; g_fade = 0.0f; }
                    else if (point_in(mx,my,g_wiz.edit_x[2],g_wiz.edit_y[2],g_wiz.edit_w,g_wiz.edit_h)){ current_state = WIZ_PREFS; g_step_anim_dir = -1; g_step_anim_t = WIZ_ANIM_MS; g_fade_raw = 0.0f; g_fade = 0.0f; }
                }
            }
        }

        // compute current slide offset from time-based animation state.
        // ease-out cubic over WIZ_ANIM_MS, starting from +/-220 px.
        int slide_dx = 0;
        if (g_step_anim_t > 0 && g_step_anim_dir != 0){
            float t = 1.0f - ((float)g_step_anim_t / (float)WIZ_ANIM_MS);
            float e = ease_out_cubic(t);
            int offset = (int)((1.0f - e) * 220.0f);
            slide_dx = (g_step_anim_dir > 0) ? offset : -offset;
        }

        // render at ~60fps but force one frame immediately after any input/state
        // change so keystrokes feel instantaneous. We track "dirty" cheaply by
        // any keypress, click, state change, or an active animation.
        bool any_anim = (g_step_anim_t > 0) || (g_shake_t > 0) || (g_clock_pulse_t > 0)
                        || (current_state == FADE_OUT) || (g_fade_raw < 1.0f && current_state != IDLE);
        // also dirty whenever any per-dot pop animation is still ticking
        if (!any_anim){
            for (int i = 0; i < 64 && !any_anim; i++){
                if (g_login_dot_pop[i] > 0 || g_reg_pw_dot_pop[i] > 0 || g_reg_cf_dot_pop[i] > 0) any_anim = true;
            }
            for (int i = 0; i < 16 && !any_anim; i++) if (g_reg_pin_dot_pop[i] > 0) any_anim = true;
        }
        bool force_frame = clicked || enter_pressed || esc_pressed || Keyboard::HasChar();
        if (g_frame_accum >= 16 || force_frame || any_anim){
            g_frame_accum = 0;
            ...
            int shake = 0;
            if (current_state == SHAKE && g_shake_total > 0){
                // smooth decaying sine-like shake using cubic easing on remaining time
                float remain = (float)g_shake_t / (float)g_shake_total;
                float amp = 10.0f * remain;
                int phase = (g_shake_total - g_shake_t) / 30;
                shake = (phase % 2 == 0) ? -(int)amp : (int)amp;
            }
            ...
            switch (current_state){
                ...
                case WIZ_PROFILE: draw_wizard_step1(slide_dx); break;
                case WIZ_SECURITY:draw_wizard_step2(slide_dx); break;
                case WIZ_PREFS:   draw_wizard_step3(slide_dx); break;
                case WIZ_SUMMARY: draw_wizard_step4(slide_dx); break;
            }
            ...
            // CRITICAL: restore the framebuffer pointer even if backbuffer was null
    ...
    for (int i = 0; i < 64; i++){ g_login_pw[i] = 0; g_reg.password[i] = 0; g_reg.confirm[i] = 0; }
    for (int i = 0; i < 16; i++) g_reg.pin[i] = 0;
    g_login_pw_len = 0; g_reg.pw_len = 0; g_reg.cf_len = 0; g_reg.pin_len = 0;
