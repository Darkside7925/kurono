...
void LinuxDriverFramework::RegisterGPUDrivers() {
    ...
    // DRM KMS (Kernel Mode Setting) — supports 1024x768, 1920x1080, 2560x1440
    memset(&drv, 0, sizeof(drv));
    lf_scpy(drv.name, "drm_kms", LDRV_MAX_NAME);
    lf_scpy(drv.description, "DRM Kernel Mode Setting (BGA)", LDRV_MAX_DESC);
    lf_scpy(drv.version, "1.0.0", 16);
    lf_scpy(drv.license, "GPL", 16);
    drv.category = LDRV_CAT_GPU;
    drv.major = 226;
    ...
    // VGA console
    memset(&drv, 0, sizeof(drv));
    lf_scpy(drv.name, "vgacon", LDRV_MAX_NAME);
    lf_scpy(drv.description, "VGA text console driver", LDRV_MAX_DESC);
    lf_scpy(drv.version, "1.0.0", 16);
    lf_scpy(drv.license, "GPL", 16);
    drv.category = LDRV_CAT_GPU;
    drv.probe = probe_vga;
...
void LinuxDriverFramework::RegisterSoundDrivers() {
    ...
    lf_scpy(drv.description, "Sound Blaster 16 (ALSA)", LDRV_MAX_DESC);
    lf_scpy(drv.version, "1.0.0", 16);
    lf_scpy(drv.license, "GPL", 16);
