...
// ── Packet reception callback from E1000 ─────────────────────────────────
...
void Network::Init() {
    ...
    // ── Detect and initialize REAL E1000 NIC ────────────────────────────
...
// ── Packet I/O (real E1000 + fallback) ───────────────────────────────────
...
void WiFi::SimulateNetworks() {
    ...
    auto add_net = [](const char* ssid, int signal, int channel, WiFiSecurity sec) {
        if (WiFi::network_count >= NET_MAX_WIFI_NETS) return;
        WiFiNetwork& n = WiFi::networks[WiFi::network_count++];
        ncpy(n.ssid, ssid, NET_MAX_SSID);
        n.signal_strength = signal;
        n.channel = channel;
        n.security = sec;
        n.connected = false;
        nmemset(&n.bssid, 0, 6);
        n.bssid.bytes[0] = 0xAA;
        n.bssid.bytes[1] = (unsigned char)(WiFi::network_count * 17);
        n.bssid.bytes[2] = (unsigned char)(WiFi::network_count * 31);
    };

    // If we have a real E1000 NIC, the first entry represents the real connection
    if (E1000::IsDetected() && E1000::IsLinkUp()) {
        add_net("QEMU-Bridge",     -25,  6, WIFI_WPA2);
        // Mark as auto-connected if link is up
        if (WiFi::network_count > 0) {
            WiFi::networks[0].connected = true;
            WiFi::connected_index = 0;
        }
    }

    // Virtual networks for UI demonstration
    add_net("Kurono-WiFi",      -35,  6, WIFI_WPA2);
    add_net("HomeNetwork",      -52,  1, WIFI_WPA2);
    add_net("CoffeeShop-Free",  -68, 11, WIFI_OPEN);
    add_net("Neighbor-5G",      -72,  6, WIFI_WPA3);
    add_net("Office-Secure",    -55,  1, WIFI_WPA2);
    add_net("IoT-Network",      -80, 11, WIFI_WEP);
    add_net("Guest-Access",     -45,  6, WIFI_WPA);
}
