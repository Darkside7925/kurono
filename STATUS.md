...
- **Linux syscall ABI completeness pass (Tier 1-11).** The whole x86_64 syscall surface from the completeness spec is now **reachable** from the amd64 `SYSCALL` fast path. The dominant gap was not missing logic but missing *wiring*: ~140 handlers already lived in `LinuxSyscall::Dispatch` yet the `SYSCALL` entry only forwarded the numbers listed in its translation table, so splice/fallocate/the sched family/the uid-gid family/xattr/ptrace/etc. all returned `-ENOSYS` at the translation miss even though `Dispatch` could service them. The `kNrMap` in `src/linux/linux_syscall_x64.cpp` grew to **231 wired amd64 numbers**, and the genuinely-missing ones got real handlers: `mremap` (grow-with-move actually copies), `mincore`, `statfs`/`fstatfs`, the full uid/gid identity family (`setuid`/`setresuid`/`getresuid`/`setfsuid`/...), `sched_setaffinity`/`getaffinity`/`setscheduler`/`getattr`/`get_priority_max`, `setpriority`/`getpriority`, `linkat`/`symlinkat`/`fchmodat`/`utimensat`, `sendmmsg`/`recvmmsg`, `kill`/`tkill`/`rt_sigqueueinfo`/`rt_sigpending`, `times`/`getitimer`, `dup3`, `preadv`/`pwritev`, `openat2`, `pidfd_send_signal`, and more. Sandbox/namespace/advisory/NUMA/xattr calls are **contract-stubs** (the exact value real Linux returns when the feature is unavailable, e.g. `getxattr → -ENODATA`), so they are never a blind `-ENOSYS` a caller chokes on. Only `ptrace` (non-TRACEME, needs a trap mechanism), `add_key`, and `request_key` (no kernel keyring) return honest `-ENOSYS`. A permanent **rate-limited `[kls] ENOSYS nr=<n>` serial trace** on every unimplemented hit is the strace-equivalent audit hook. **Verified headless** (`kurono.klstest`, `-smp 1`): a per-tier self-test drives `Dispatch` and logs **30/30 PASS** (incl. the real memory work, the identity round-trips, the xattr errno contract, the signal-mask read-back, and the ENOSYS logger firing exactly once); the musl dynamic-linker path (`kurono.dyntest`) still resolves libc and runs to **`rc=0`** with the only boot-time ENOSYS being the test's own deliberate ptrace probe. Full per-syscall table: `docs/developers/linux/SYSCALLS.md`. **Honest:** io_uring is staged (real fd, ring not yet shared-mapped); signals are post-only (no async delivery, so signalfd/pause can't fire a handler); the stubs satisfy the call shape, they don't add the missing subsystem.
- **Firefox now runs a full Wayland client handshake against the in-kernel compositor (the window is one fix away).** A real Firefox 140.11.0esr (musl, `--disable-jit`) went from a hard early-init deadlock to a complete Wayland client bringup: it connects to the compositor, its WaylandProxy forwards `get_registry`, it binds all 9 advertised globals, and it runs `wl_compositor.create_surface` then `wl_shm.create_pool` (buffer allocation) then `wl_seat` get_pointer/get_keyboard (input) then `wl_output`, verified headless with **0 faults and 0 protocol errors**. Six fixes got it there (branch `feat/firefox-poll-deschedule`): a cooperative `poll`/`ppoll` that yields a blocked user thread to its siblings mid-syscall (this broke the `pthread_cond_wait` deadlock where one user thread monopolized the cpu and the sibling clone threads never ran); clone threads sharing the thread-group leader's mmap region table (a worker stack-growth SIGSEGV); accepted AF_UNIX sockets inheriting the kernel server's data callback; CLONE_FILES threads sharing one fd table + a listen-socket POLLIN wake + the scheduler fix that re-readies a descheduled user-thread poll (`OnTimerTick` was returning early for any user thread, so a parked poll never woke); `ioctl(FIONREAD)` on sockets (firefox's proxy sizes its reads with it); and the compositor's `iface_id_of` exact-name match + bind-id assignment + the initial global events (`wl_shm.format` / `wl_seat.capabilities` / `wl_output`) a global must announce before gtk maps a window. **Update (xkb keymap data):** the "bind-id desync" was a misdiagnosis. The real connect-path bug was a `KernelInject` loopback (fixed on main); with that, plus shipping the **xkeyboard-config data** as a kernel blob and unpacking it at launch (gdk calls `xkb_keymap_new_from_names(evdev/pc105/us)` at `wl_seat` init, and the firefox tar ships libxkbcommon but no config data, so the keymap build aborted with SIGSEGV), firefox now clears seat-init and runs the **complete** Wayland handshake: it binds *every* global including `xdg_wm_base`, creates its `wl_surface`s, and gets pointer+keyboard, with **0 protocol errors**. **The remaining blocker is the e10s multiprocess IPC:** after the handshake firefox pegs one CPU core in a constant busy-spin (measured ~100 ticks/s, rock-steady) waiting on a child-process IPC reply that never arrives, and never maps its toplevel. The children spawn and stay alive; the parent's AF_UNIX-socketpair/shm IPC across `fork`/`exec` is the open frontier. **Honest:** still not a rendered window, but the Wayland render path itself works (a musl Wayland client renders end to end), so this is now firefox-internal multiprocess IPC, not the compositor or the wire protocol.
- **Installed packages survive reboot (`/apps` persistence).** `PersistStore` now mirrors `/apps` to the nvme data disk alongside `/home`/`/etc`/`/root`, and a fresh firefox install is saved right after extraction, so the next boot restores it instead of re-downloading + re-extracting (~262 MB): firefox launches at **~30s on reuse** vs a ~60-145s reinstall, and `restore_subtree` now logs any dropped/truncated file instead of silently skipping it. **Honest:** the *reuse* path currently stalls firefox in early init before the wayland connect (the install path connects fine and the restore is byte-complete, so it is a boot-timing / heap-state interaction at early boot, not the persisted files); tracked.
- **KMemX - Kurono Memory Compression Engine.** Transparently compresses inactive 4 KB pages across the whole OS into a fixed physical pool with LZ4, so a cold page costs ~1.5-2 KB instead of 4 KB and is decompressed back on the page fault that next touches it. A dedicated, CPU-capped kernel-process (`PRIO_HIGH`, 10 ms tick, **token bucket of 1-16 pages/tick by pressure** so CPU stays at 1-5%; yields to the compositor when frames are pending) ages pages via a **4-bit generation counter parked in spare PTE bits 52-55** (an LRU clock off the hardware Accessed bit) and compresses ones past the pressure-derived threshold. A compressed page's leaf goes NOT-present + `PTE_KMEMX_COMPRESSED`; the `#PF` handler (hooked in `hal.cpp` before demand-zero) decompresses it back into a fresh frame, **CRC32-verifies it (mismatch -> kernel panic, never hands back wrong bytes)**, and restores the leaf. The LZ4 codec is a clean-room freestanding block-format implementation with **zero heap allocation on the hot path**, an **SSE2 `movdqu` wildcopy** + **slice-by-8 CRC32**; the pool is 2 MB chunks with a best-fit coalescing free-list, bounded so it never drives the system to OOM; the metadata table is a flat open-addressed hash keyed `(address_space, vaddr)` with backward-shift deletion. An adaptive **pressure system (green/yellow/orange/red/critical)** integrates with kinit (compress guests at orange+, stop launching services at red, shed lowest-priority services at critical); **KSM-style dedup** with COW refcounting merges identical pages; **hypervisor guest pages** are compressed at the EPT level (wired into the EPT-violation handler). User-facing: an enable/disable **toggle + disclaimers** (config `kmemx.enabled` / `kmemx.conf`, default on), a **Kurono Setup installer option**, a **Settings -> System -> Memory Compression panel** (toggle + pool-size + aggressiveness sliders + a usage gauge), and a **`kmemx` shell command** (status/stats/pressure/compress/decompress/flush/config/enable/disable/benchmark). **Verified headless** (`kurono.kmemxtest`, `-m 2G`, KVM): **SUMMARY 7/7** - 1000-page LZ4 roundtrip byte-exact (also vs an independent reference decoder + 2M-op metadata-table fuzz + 500k-op pool fuzz on the host), pool alloc/coalesce, 1000-page store/retrieve through the real pool, PTE aging, an **end-to-end compress -> real `#PF` -> decompress** cycle (64/64 byte-exact), and dedup (no leak/double-free). **Fault decompression latency on an idle host: ~1.9 us warm / ~3.9 us mean / ~8.2 us max, 0 over the 10 us budget** (the SSE2 + slice-by-8 work cut the mean from ~12 us). Compression ratio ~157:1 zero pages, ~45:1 structured, ~2.2:1 over the mixed test corpus. **Honest:** the full-system targets (Firefox 500 MB -> 50 MB, guests -70%) are aspirational and need those real workloads booted under the engine; the native compress/fault pipeline they ride on is fully verified, but the EPT guest-compression path can't be exercised headless without a running Alpine/Debian guest, so it is built + wired but unverified end-to-end. Under heavy host load the virtualized TSC inflates the latency figures (the idle-host numbers above are representative). See `docs/developers/kernel/KMEMX.md`.
...
- **Firefox's Gecko engine runs on Kurono.** Real **Firefox 140.11.0esr** built against **musl** (clang/lld, `--disable-jit`, `--disable-sandbox`, `cairo-gtk3-wayland`) → `firefox`/`firefox-bin` + a 174 MB `libxul.so` Gecko engine, build rc=0, all 12 Alpine musl portability patches applied. The launcher (real `execve` of `/apps/firefox/firefox`), the syscall/IPC/Wayland surface, and the on-device loader are in place: `ld-kurono` resolves libxul's full `.so` dependency closure and loads + relocates `libxul` (130 MB+) at a multi-terabyte base, and **XPCOM + Gecko's own application code execute**, including spawning Firefox's child processes (via `clone3`). Since then the runtime was driven all the way to a full Wayland client handshake (see the top "What's New" entry): firefox binds all 9 globals and runs create_surface + a `wl_shm` pool + `wl_seat` + `wl_output` with 0 faults. The remaining blocker before a **rendered window** is the e10s multiprocess IPC: firefox completes the *full* handshake (after the xkb keymap-data fix it also binds `xdg_wm_base` and creates its surfaces), then busy-spins one CPU core on a child-process IPC reply that never arrives. This is *not* an address-space limit (the old `<4 GB` user-pointer ABI cap is lifted; user mappings span the full canonical 64-bit user range). This is the concrete rebuttal to "a freestanding OS can't have a browser."
...
- [~] **Browser**  -  the GUI tile is a deliberate placeholder (use `curl <url>` today). The real browser path is **Firefox on the Linux runtime**: Firefox 140.11.0esr is cross-compiled against musl + Wayland (see *Firefox-Class Userspace Runtime* below), the OS provides the syscalls/IPC/Wayland it targets, and on-device `ld-kurono` already loads libxul's full closure so the **Gecko engine runs** (XPCOM + Gecko app code + child-process spawn). What's left is a **rendered window**: firefox now completes the full Wayland handshake (xkb keymap data shipped) but then busy-spins on the e10s multiprocess IPC before mapping its toplevel; not the old pointer-ABI limit (that cap is lifted).
...
### Linux Subsystem
- [x] **280 syscall numbers** defined (`LSYS_*` in `src/linux/linux_syscall.h`); **304 handler arms** in `src/linux/linux_syscall.cpp`
- [x] **Full x86_64 ABI completeness (Tier 1-11) reachable** -- 231 amd64 numbers wired through the `kNrMap` translation table in `src/linux/linux_syscall_x64.cpp` (the gap was handlers existing but unreachable from the `SYSCALL` fast path), plus stub-ok + direct-handled numbers. No spec syscall returns a blind `-ENOSYS`. Real: read/write/open/close/lseek/brk/stat/fstat/mmap/mremap/munmap/mprotect, the uid-gid identity family, sched affinity/nice/policy, futex, clone/clone3, epoll/poll, memfd, copy_file_range, pidfd, mincore, statfs, sendmmsg/recvmmsg, sendmsg/recvmsg (SCM_RIGHTS), and more; contract-stubs for sandbox/namespace/advisory/NUMA/xattr; honest `-ENOSYS` only for ptrace(non-TRACEME)/add_key/request_key. Per-syscall table in `docs/developers/linux/SYSCALLS.md`
- [x] **Permanent rate-limited `[kls] ENOSYS nr=<n>` audit trace** on every unimplemented hit (strace-equivalent), and a `kurono.klstest` per-tier self-test (30/30 PASS headless); the musl dynamic-linker path (`kurono.dyntest`) still runs to `rc=0` after the build-out
